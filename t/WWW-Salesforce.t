...
# tests -- describeTabs
{
    ...
    SKIP: {
        skip( "Can't check tabs results since describeTabs failed", 2 ) unless $passed;
...
#test -- query
{
    ...
    #test -- queryMore
    SKIP: {
        my $locator = $res->valueof('//queryResponse/result/queryLocator') if $res;
...
{
    my $res = $sforce->query(
        'query' => 'Select a.CreatedBy.Username, a.Name From Account a limit 2'
    );
    my $passed = 0;
    $passed = 1 if (
        $res && $res->valueof('//done') eq 'true'
    ...
    ok(
        defined($recs[0]->{'Name'}) && defined($recs[0]->{'CreatedBy'}->{'Username'}),
        "relational query - first record check"
    );

    # test -- second check for expected structure
    ok(
        defined($recs[1]->{'Name'}) && defined($recs[1]->{'CreatedBy'}->{'Username'}),
...
# test -- create a record
{
    my $res = $sforce->create( 
        'type'=>'Account', 'Name'=>'foobar test account'
    );
    my $passed = 0;
    ...
    SKIP: {
        skip( "can't update and delete new account since the creation failed", 2 ) unless $passed;
        #test -- update
        my $id = 0;
        $id = $res->valueof('//id') if $passed;
        $res = $sforce->update(
            'type'=>'Account', 'id'=>$id, 'Name'=>'foobar test account updated'
        );
        $passed = 0;
        $passed = 1 if ( $res->valueof('//success') eq 'true' && defined($res->valueof('//id') ) );
        ...
        ok(
            $res->valueof('//success') eq 'true' && defined($res->valueof('//id') ),
...
# tests -- base64 doc files
{
    ...
    SKIP: {
        skip( "Can't create a document since I can't get the folder ID", 1 ) unless $fid;
    ...
    SKIP: {
        skip( "Can't query for the document we just tried to create because we couldn't determine the ID", 1 ) unless $docid;
        $res = $sforce->query( 
            'type' => 'Document', 'query'=>"select id,body from Document where Id = '$docid' limit 1"
        );
        $doc = $res->valueof('//records')
            if ( defined($res->valueof('//records')) && $res->valueof('//size') eq '1' );
    ...
    SKIP: {
        skip( "Can't delete image because it wasn't created properly", 1 ) unless $docid;
        my @toDel = ($docid);
        $res = $sforce->delete(@toDel);
...
#tests -- create and mass update some contacts
{
    ...
    SKIP: {
        skip( "No point creating a second account since the first failed", 1 ) unless $oneid;
    ...
    SKIP: {
        skip( "No point trying a multiple update since we couldn't create multiple contacts", 1 )
            unless $oneid && $twoid;
        $res = $sforce->update(
            type=>'Contact', 
    ...
    SKIP: {
        ...
        ok( defined($results[0]) && defined($results[1]) && $#results == 1 , "multi-update batch results check");
    }

    #test -- cleanup the temp contact records
    SKIP: {
        skip( "no results to delete from the mult-update batch", 1 ) unless ($oneid || $twoid);
        if ( $oneid && $twoid ) {
        ...
        ok( $res && $res->valueof('//success') eq 'true', "multi-update batch deletion")
            or diag( $! );
    }
}
